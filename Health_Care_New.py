...
class QuestionDigonosis(Frame):
    ...
    def createWidget(self):
        self.bg_Photo = ImageTk.PhotoImage(Image.open('health-care_image.jpg'))
        self.bg_label = ttk.Label(self, image=self.bg_Photo)
        self.bg_label.grid(row=0,column=0)
        self.label=Label(self.bg_label)
        self.label.place(x=5,y=4)
        self.lblQuestion = Label(self.label, text="Question", width=12, bg="bisque",font='bold')
        self.lblQuestion.grid(row=0, column=0, rowspan=4)

        self.lblDigonosis = Label(self.label, text="Digonosis", width=12, bg="bisque",font='bold')
        self.lblDigonosis.grid(row=4, column=0, sticky="n", pady=5)

        self.varQuestion=StringVar()
        self.txtQuestion = Text(self.label, width=100, height=4,font='bold')
        self.txtQuestion.grid(row=0, column=1, rowspan=4, columnspan=20)

        self.varDiagonosis = StringVar()
        self.txtDigonosis = Text(self.label, width=100, height=14,font='bold')
        self.txtDigonosis.grid(row=4, column=1, columnspan=20, rowspan=20, pady=5)

        self.btnNo = Button(self.label, text="No", width=12, bg="bisque", command=self.btnNo_Click,font='bold')
        self.btnNo.grid(row=25, column=0)
        self.btnYes = Button(self.label, text="Yes", width=12, bg="bisque", command=self.btnYes_Click,font='bold')
        self.btnYes.grid(row=25, column=1, columnspan=20, sticky="e")

        self.btnClear = Button(self.label, text="Clear", width=12, bg="bisque", command=self.btnClear_Click,font='bold')
        self.btnClear.grid(row=27, column=0)
        self.btnStart = Button(self.label, text="Start", width=12, bg="bisque", command=self.btnStart_Click,font='bold')
...
class MainForm(Frame):
    ...
    def createWidget(self):
        ...
        # self.lblMsg.pack()
        self.new_label = Label(self, bg='white', width='200', height='1')
        self.new_label.pack()
        self.bg_Photo = ImageTk.PhotoImage(Image.open('login_page.jpg'))
        self.bg_label = ttk.Label(self, image=self.bg_Photo)
        self.bg_label.pack()
        self.btnLogin = Button(self.new_label, text="Login", bg='white', height="1", width="20",
                               command=self.lblLogin_Click)
        self.btnLogin.pack(side='right', padx=10)
        self.btnRegister = Button(self.new_label, text="Register", bg='white', height="1", width="20",
...
class Login(Frame):
    ...
    def destroyPackWidget(self, parent):
        # Return a list of all slaves of this widget in its packing order.++(pack_slaves)
        for e in parent.pack_slaves():
            e.destroy()
    ...
    def createWidget(self):
        self.bg_Photo1 = ImageTk.PhotoImage(Image.open('login_page1.jpg'))
        self.bg_label1 = ttk.Label(self, image=self.bg_Photo1)
        self.bg_label1.grid(row=0, column=0)
        self.lblMsg = Label(self, text="Please enter details below to login", font='bold',bg='white')
        ...
        self.username_login_entry.grid(row=2, column=1,columnspan=2)
        self.password = Label(self, text="Password :", font='bold',bg='white')
        self.password.grid(row=3, column=0)
        self.password_verify = StringVar()
        self.password_login_entry = Entry(self, textvariable=self.password_verify, show='*')
        self.password_login_entry.grid(row=3, column=1)
        self.btnLogin = Button(self, text="Login", width=10, height=1, command=self.btnLogin_Click, font='bold',bg='white')
...
class SignUp(Frame):
    ...
    def createWidget(self):
        ...
        self.bg_label1 = ttk.Label(self, image=self.bg_Photo1)
        self.bg_label1.grid(row=0, column=0)
        self.lblMsg = Label(self, text="Please enter details below", font='bold',bg='white')
        self.lblMsg.grid(row=1,column=0)
        self.username_lable = Label(self, text="Username :",font='bold',bg='white')
        self.username_lable.grid(row=2,column=0)
        self.username = StringVar()
        self.username_entry = Entry(self, textvariable=self.username)
        self.username_entry.grid(row=2,column=1)

        self.password_lable = Label(self, text="Password :",font='bold',bg='white')
        self.password_lable.grid(row=3,column=0)
        self.password = StringVar()
        self.password_entry = Entry(self, textvariable=self.password, show='*')
        self.password_entry.grid(row=3,column=1)
        self.btnRegister = Button(self, text="Register", width=10, height=1, font="bold", command=self.register_user,bg='white')
        self.btnRegister.grid(row=4,column=0)
    ...
    def register_user(self):
        ...
        self.lblSucess = Label(root, text="Registration Success", fg="green", font="bold")
        self.lblSucess.pack()
        self.btnSucess = Button(root, text="Click Here to proceed", command=self.btnSucess_Click,font='bold')
        self.btnSucess.pack()
